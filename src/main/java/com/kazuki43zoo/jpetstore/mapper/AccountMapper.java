...
interface AccountMapper {
    ...
    @Insert("INSERT INTO ACCOUNT(EMAIL, FIRSTNAME, LASTNAME, STATUS, ADDR1, ADDR2, CITY, STATE, ZIP, COUNTRY, PHONE, USERID) VALUES(#{email}, #{firstName}, #{lastName}, #{status}, #{address1}, #{address2}, #{city}, #{state}, #{zip}, #{country}, #{phone}, #{username})")
    void insertAccount(Account account);

    @Insert("INSERT INTO PROFILE(LANGPREF, FAVCATEGORY, MYLISTOPT, BANNEROPT, USERID) VALUES(#{languagePreference}, #{favouriteCategoryId}, #{listOption}, #{bannerOption}, #{username})")
    void insertProfile(Account account);

    @Insert("INSERT INTO SIGNON(PASSWORD, USERNAME)VALUES(#{password}, #{username})")
    void insertSignon(Account account);

    @Update(" UPDATE ACCOUNT SET EMAIL= #{email}, FIRSTNAME = #{firstName}, LASTNAME = #{lastName}, STATUS = #{status}, ADDR1 = #{address1}, ADDR2 = #{address2}, CITY = #{city}, STATE = #{state}, ZIP = #{zip}, COUNTRY = #{country}, PHONE = #{phone} WHERE USERID = #{username} ")
    void updateAccount(Account account);

    @Update(" UPDATE PROFILE SET LANGPREF= #{languagePreference}, FAVCATEGORY = #{favouriteCategoryId}, MYLISTOPT = #{listOption}, BANNEROPT = #{bannerOption} WHERE USERID = #{username}")
